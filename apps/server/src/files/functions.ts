...
export default {
  private: {
    ...
    getById: async (args: InferPrivateArgs["getById"]) => {
    ...
    getLinkById: async (args: InferPrivateArgs["getLinkById"]) => {
    ...
    updateById: async (args: InferPrivateArgs["updateById"]) => {
      const file = await getFileMaybe(args.Id);
      if (!file) {
        return asPayload(404, { message: "Not found" });
      }
    ...
    deleteById: async (args: InferPrivateArgs["deleteById"]) => {
      const file = await getFileMaybe(args);
      if (!file) {
        return asPayload(404, { message: "Not found" });
      }
