  ...
  const renderIconOptions = () => {
    if (location.pathname === "/") {
      return (
        <div className={"flex flex:1 justify-content:center"}>
          <IconGearWide
            onClick={() => navigator("/options")}
            size={22}
            className={
              "flex bg:white justify-content:center align-items:center  b:1px|solid|gray w:2.5em h:2.5em r:50% opacity:0.6"
            }
          />
        </div>
      );
  ...
  return (
    <nav className="flex flex:row p:1em justify-content:space-between align-items:center">
      ...
      <div className={"flex flex:6 flex-col justify-content:center"}>
        ...
        <p className={"text-align:center p:0 m:0 opacity:0.8 lh:0.6em"}>
          Monitory
        </p>
      </div>
